...
q <- ggplot(covid, aes(x = waitTime, fill = dependent)) + geom_histogram(aes(y = ..density..),alpha = 0.50, binwidth = 1) +
  ...
  xlab("Wait Time (Days)") + ylab("Density of Cases") + xlim(0,20) +geom_vline(xintercept = 3.185575, size = 0.5,alpha = 0.25, colour = "red", linetype = "solid") + 
  geom_vline(xintercept = 3.330703, size = 0.5,alpha = 0.5, colour = "turquoise", linetype = "solid")
